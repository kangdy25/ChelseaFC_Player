...
app.post("/player/:season/:name/crawl", async (req, res) => {
  ...
  try {
    ...
    profileValues = await page.$$eval(
      ".profile-player-details__item-value",
    ...
    const seasonValues = await page.$$(`.dropdown__item`, (els) =>
    ...
    const statValues = await page.$$eval(".player-stat__value", (els) =>
      els.map((el) => el.textContent.trim())
    );
    ...
    const yellowCard = await page.$eval(
      ".stats-fouls__yellow-cards__value",
      (el) => el.textContent
    );
    await page.waitForSelector(".stats-fouls__red-cards__value");
    const redCard = await page.$eval(
      ".stats-fouls__red-cards__value",
      (el) => el.textContent
    );
    ...
    if (playerData.role !== 0) {
      ...
      const goalsOutside = await page.$eval(
        ".stats-goals__box-stats__values-out",
        (el) => el.textContent
      );

      await page.waitForSelector(".stats-goals__box-stats__values-in");
      const goalsInside = await page.$eval(
        ".stats-goals__box-stats__values-in",
        (el) => el.textContent
      );

      await page.waitForSelector(".stats-scored-with__head__value");
      const scoredWithHead = await page.$eval(
        ".stats-scored-with__head__value",
        (el) => el.textContent
      );

      // chelsea.com 공식 홈페이지의 셀렉터 표시 오류 (왼발-오른발)
      await page.waitForSelector(".stats-scored-with__right-foot__value");
      const scoredWithLeft = await page.$eval(
        ".stats-scored-with__right-foot__value",
        (el) => el.textContent
      );

      // chelsea.com 공식 홈페이지의 셀렉터 표시 오류 (왼발-오른발)
      await page.waitForSelector(".stats-scored-with__left-foot__value");
      const scoredWithRight = await page.$eval(
        ".stats-scored-with__left-foot__value",
        (el) => el.textContent
      );

      await page.waitForSelector(".stats-scored-with__penalty__value");
      const penalties = await page.$eval(
        ".stats-scored-with__penalty__value",
        (el) => el.textContent
      );

      await page.waitForSelector(".stats-scored-with__free-kick__value");
      const freeKicks = await page.$eval(
        ".stats-scored-with__free-kick__value",
        (el) => el.textContent
      );
    ...
    if (playerData.role === 0) {
      ...
      const savesOutside = await page.$eval(
        ".stats-goalkeeping__box-stats__values-out",
        (el) => el.textContent
      );

      await page.waitForSelector(".stats-goalkeeping__box-stats__values-in");
      const savesInside = await page.$eval(
        ".stats-goalkeeping__box-stats__values-in",
        (el) => el.textContent
      );
    ...
    const passCompletion = await page.$$eval(
      ".stats-pass-completion__value",
      (els) => els.map((el) => el.textContent.trim())
    );

    await page.waitForSelector(
      ".stats-pass-success__player-rank__percentage-value"
    );
    const passSuccess = await page.$eval(
      ".stats-pass-success__player-rank__percentage-value",
      (el) => el.textContent
    );
