...
const PlayerStatPage = () => {
  ...
  const playerData = seasonInfo
    .find((seasonArray) =>
      seasonArray.some((player) => player.season === parseInt(season))
    )
    ?.find((player) => player.url_name === name);
    ...
    async (playerData) => {
      try {
        const response = await fetch(
          `${apiUrl}/player/${season}/${name}/crawl`,
          {
            method: "POST",
            headers: {
